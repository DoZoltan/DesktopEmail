...
namespace SaintSender.Core.Services
{
    public class CredentialService
    {
        ...
        private AesCryptoServiceProvider CreateAesCryptoServiceProvider()
        {
            ...
            aesCryptoServiceProvider.Key = Encoding.UTF8.GetBytes("LóriZoliRoli");
            aesCryptoServiceProvider.IV = Encoding.UTF8.GetBytes("RoliZoliLóri");
        ...
        public string[] GetSavedCredentials()
        {
            ...
            {
                using (IsolatedStorageFileStream isolatedStream = new IsolatedStorageFileStream("login_cred.data", FileMode.Open))
                using (StreamReader reader = new StreamReader(isolatedStream))
                using (MemoryStream ms = new MemoryStream())
                {
                    reader.BaseStream.CopyTo(ms);
                    byte[] encryped = ms.ToArray();
                    string decripted = DecryptStringFromBytes_Aes(encryped, _myAes.Key, _myAes.IV);
                    string[] cred = decripted.Split('/');
                    return cred;
                }
            }
            else { return null;  }
